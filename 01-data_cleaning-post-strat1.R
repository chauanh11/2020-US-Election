...
reduced_data$household_income <- reduced_data$ftotinc
...
reduced_data$race <- gsub("two major races", "Some other race", reduced_data$race)
reduced_data$race <- gsub("three or more major races", "Some other race", reduced_data$race)
reduced_data$race <- gsub("other race, nec", "Some other race", reduced_data$race)
reduced_data$race <- as.factor(reduced_data$race)
reduced_data$race_ethnicity <- reduced_data$race
...
reduced_data$education <- reduced_data$educd
...
reduced_data$gender <- reduced_data$sex
...
census_data %>%
  ggplot(aes(age, prop.table(table(vote_trump)), fill=sex))
